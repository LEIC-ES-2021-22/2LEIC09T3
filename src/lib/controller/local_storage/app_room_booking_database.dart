...
class AppBookingsDatabase extends AppDatabase {
  AppBookingsDatabase()
      : super('room_bookings.db', [
          'CREATE TABLE room_bookings(bookingId INTEGER PRIMARY KEY, state TEXT, room TEXT, duration INTEGER, date TEXT)'
        ]);
  ...
  Future<void> insertRoomBookings(List<RoomBooking> bookings) async {
    for (RoomBooking booking in bookings) {
      await insertInDatabase('room_bookings', booking.toMap(),
    ...
    return List.generate(maps.length, (i) {
      return RoomBooking(
          ...
          BookingState.values.firstWhere(
              (element) => element.name.toString() == maps[i]['state']),
          maps[i]['room'],
          maps[i]['duration'],
